...
[gotcha] dev-runner-edge: WXT's dev runner uses chrome-launcher, which only finds Google Chrome → "No Chrome installations found" on Edge-only machines. Fixed in wxt.config.ts via `webExt.binaries` pointing at an Edge binary (env `WXT_BROWSER_BINARY` overrides; per-platform Edge fallback; undefined ⇒ default Chrome lookup). web-ext passes it as chrome-launcher `chromePath`, skipping the Chrome search. Edge runs the chrome-mv3 build.
