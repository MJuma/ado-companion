...
[gotcha] no-display-none-on-ado: do NOT set display:none on ADO's native content to hide it under our overlay — it breaks ADO's rendering across file/view switches and leaves a BLANK pane when you exit. Use a pure opaque absolute overlay (background var(--background-color); the native content stays laid out, just covered) so exiting always reveals it intact.
