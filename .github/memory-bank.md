...
[pattern] feature-progress: Review MVP complete + UX-refined. Native "Review" entry in ADO's view dropdown; island overlays the file-content pane (tree preserved); comment by selecting text (floating + button → composer popover anchored to source lines); composer autofocuses, shows a live preview, and pastes images (no image button); selecting another view exits. Settings/allowlist gate the host. Future: lazy-load heavy deps, more ADO surfaces.
